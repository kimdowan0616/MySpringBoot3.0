...
import com.basic.myspringboot.dto.UserReqFormDTO;
...
@Service
@RequiredArgsConstructor    // final 변수들에 생성자를 대신해주는 어노테이션. Constructor Injection을 간편하게.
@Transactional
...
//    }
    ...
    public List<UserResDTO> getUsers() {
        ...
        List<UserResDTO> userResDTOList = userList.stream()   // Stream<User>
                // map(Function) Function의 추상 메서드 R apply(T t)
    ...
    public void updateUserForm(UserReqFormDTO userReqFormDTO) {
        User existUser = userRepository.findById(userReqFormDTO.getId())
                .orElseThrow(() -> new BusinessException(userReqFormDTO.getId() + " User Not Found", HttpStatus.NOT_FOUND));

        existUser.setName(userReqFormDTO.getName());
    }
